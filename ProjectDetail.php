...
if ($role_session == 1) {
	echo "<form action=\"#\" method=\"post\"><input type=\"submit\" class=\"btn btn-warning\" name=\"disablep\" value=\"Disable Project\"/></form>";
...
<h4><p class="text-primary">Scrum Masters:</p></h4>
<?php
...
if (mysqli_num_rows ( $scrummasters ) > 0) {
	while ( $row = mysqli_fetch_row ( $scrummasters ) ) {
		echo "<p class=\"text-info\">$row[0]</p>";
	}
} else {
	echo "<p class=\"text-danger\">No Scrum Masters added</p>";
}
echo "<h4><p class=\"text-primary\">Team members:</p></h4>";
...
if (mysqli_num_rows ( $scrummasters ) > 0) {
	...
	echo "<p class=\"text-danger\">No Team members added</p>";
}
